...
int ml307r_init(void) {
  ...
  DEBUG_4G_PRINTF("[ML307R] >>> ml307r_init start\r\n");

  // 1. AT 通信测试
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT\r\n");
  ret = at_send_command("AT", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT resp: %s, ret=%d\r\n", resp, ret);
  if (ret != 0) {
    DEBUG_4G_PRINTF("[ML307R] !!! AT test failed !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }
  DEBUG_4G_PRINTF("[ML307R] OK - AT test passed\r\n");

  // 2. 关闭回显 ATE0
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: ATE0\r\n");
  ret = at_send_command("ATE0", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< ATE0 resp: %s, ret=%d\r\n", resp, ret);
  if (ret != 0) {
    DEBUG_4G_PRINTF("[ML307R] !!! ATE0 failed !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }
  DEBUG_4G_PRINTF("[ML307R] OK - Echo disabled\r\n");

  // 3. 检查 SIM 卡 AT+CPIN?
  s_ml_state = ML307R_STATE_SIM_CHECK;
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+CPIN?\r\n");
  ret =
      at_send_command("AT+CPIN?", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+CPIN? resp: %s, ret=%d\r\n", resp, ret);
  if (ret != 0 || strstr(resp, "READY") == NULL) {
    DEBUG_4G_PRINTF("[ML307R] !!! SIM card not ready !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }
  DEBUG_4G_PRINTF("[ML307R] OK - SIM card ready\r\n");

  // 4. 等待网络注册 AT+CEREG?（最多 30 次，每次 1s）
  s_ml_state = ML307R_STATE_REGISTERED;
  DEBUG_4G_PRINTF(
      "[ML307R] >>> Waiting for network registration (AT+CEREG?)\r\n");
  int retry = 0;
  while (retry < 30) {
    DEBUG_4G_PRINTF("[ML307R] >>> AT+CEREG? attempt %d/30\r\n", retry + 1);
    ret = at_send_command("AT+CEREG?", "OK", AT_TIMEOUT_DEFAULT, resp,
                          sizeof(resp));
    DEBUG_4G_PRINTF("[ML307R] <<< AT+CEREG? resp: %s, ret=%d\r\n", resp, ret);
    if (ret == 0 && (strstr(resp, "+CEREG: 0,1") != NULL ||
                     strstr(resp, "+CEREG: 1,1") != NULL)) {
      DEBUG_4G_PRINTF("[ML307R] OK - Network registered after %d attempts\r\n",
                      retry + 1);
      break;
    }
    retry++;
  ...
  if (retry >= 30) {
    DEBUG_4G_PRINTF("[ML307R] !!! Network registration timeout !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }

  // 5. 激活 PDP 拨号 AT+MIPCALL=1,1
  s_ml_state = ML307R_STATE_DIAL;
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+MIPCALL=1,1\r\n");
  ret = at_send_command("AT+MIPCALL=1,1", "OK", AT_TIMEOUT_LONG, resp,
                        sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+MIPCALL=1,1 resp: %s, ret=%d\r\n", resp,
                  ret);
  if (ret != 0) {
    DEBUG_4G_PRINTF("[ML307R] !!! PDP activation failed !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }

  s_ml_state = ML307R_STATE_CONNECTED;
  DEBUG_4G_PRINTF("[ML307R] OK - ML307R init success, state=CONNECTED\r\n");
  return 0;
}
...
ml307r_state_t ml307r_get_state(void) {
  DEBUG_4G_PRINTF("[ML307R] >>> ml307r_get_state: %d\r\n", s_ml_state);
  return s_ml_state;
}
...
int ml307r_get_signal_quality(signal_quality_t *sq) {
  ...
  char resp[128];
  int rssi = 99, ber = 99;
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+CSQ\r\n");
  int ret =
      at_send_command("AT+CSQ", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+CSQ resp: %s, ret=%d\r\n", resp, ret);
  if (ret == 0) {
    (void)sscanf(resp, "+CSQ: %d,%d", &rssi, &ber);
  }
  sq->rssi = rssi;
  sq->ber = ber;
  DEBUG_4G_PRINTF("[ML307R] Signal quality: RSSI=%d, BER=%d\r\n", rssi, ber);
  return (ret == 0) ? 0 : -1;
}
...
bool ml307r_is_arrears(void) {
  ...
  DEBUG_4G_PRINTF("[ML307R] ml307r_is_arrears: %s\r\n", arrears ? "YES" : "NO");
  return arrears;
}
...
int ml307r_reconnect(void) {
  char resp[128];
  DEBUG_4G_PRINTF("[ML307R] >>> ml307r_reconnect start\r\n");

  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+CFUN=0\r\n");
  at_send_command("AT+CFUN=0", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+CFUN=0 resp: %s\r\n", resp);
  delay_ms(500);

  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+CFUN=1\r\n");
  at_send_command("AT+CFUN=1", "OK", AT_TIMEOUT_DEFAULT, resp, sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+CFUN=1 resp: %s\r\n", resp);
  delay_ms(2000);

  int retry = 0;
  while (retry < 30) {
    DEBUG_4G_PRINTF("[ML307R] >>> AT+CEREG? reconnect attempt %d/30\r\n",
                    retry + 1);
    int ret = at_send_command("AT+CEREG?", "OK", AT_TIMEOUT_DEFAULT, resp,
                              sizeof(resp));
    DEBUG_4G_PRINTF("[ML307R] <<< AT+CEREG? resp: %s, ret=%d\r\n", resp, ret);
    if (ret == 0 && (strstr(resp, "+CEREG: 0,1") != NULL ||
                     strstr(resp, "+CEREG: 1,1") != NULL)) {
      DEBUG_4G_PRINTF(
          "[ML307R] OK - Network re-registered after %d attempts\r\n",
          retry + 1);
      break;
    }
    retry++;
  ...
  if (retry >= 30) {
    DEBUG_4G_PRINTF("[ML307R] !!! Network re-registration timeout !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }
  ...
  s_ml_state = ML307R_STATE_REGISTERED;
  DEBUG_4G_PRINTF("[ML307R] >>> AT send: AT+MIPCALL=1,1\r\n");
  int ret = at_send_command("AT+MIPCALL=1,1", "OK", AT_TIMEOUT_LONG, resp,
                            sizeof(resp));
  DEBUG_4G_PRINTF("[ML307R] <<< AT+MIPCALL=1,1 resp: %s, ret=%d\r\n", resp,
                  ret);
  if (ret != 0) {
    DEBUG_4G_PRINTF("[ML307R] !!! PDP re-activation failed !!!\r\n");
    s_ml_state = ML307R_STATE_ERROR;
    return -1;
  }

  s_ml_state = ML307R_STATE_CONNECTED;
  DEBUG_4G_PRINTF("[ML307R] OK - ml307r_reconnect success\r\n");
  return 0;
}
...
int ml307r_mqtt_connect(void) {
  ...
  // 构造动态 Topic
  snprintf(s_topic_up, sizeof(s_topic_up), "up/%s/%s", cred->product_id,
           cred->device_id);
  snprintf(s_topic_down, sizeof(s_topic_down), "down/%s/%s", cred->product_id,
           cred->device_id);
  ...
  ret = at_mqtt_config(MQTT_SERVER, MQTT_PORT, cred->device_sn, cred->device_id,
                       cred->device_key);
  ...
  DEBUG_4G_PRINTF(
      "[MQTT] OK - ml307r_mqtt_connect success, state=CONNECTED\r\n");
  return 0;
}
...
int ml307r_mqtt_publish(const char *topic, const char *payload, int qos) {
  DEBUG_4G_PRINTF(
      "[MQTT] >>> ml307r_mqtt_publish: topic=%s, qos=%d, payload=%s\r\n", topic,
      qos, payload);
  ...
  for (int i = 0; i < n && pos < (int)sizeof(resp) - 64; i++) {
    ...
    if (code == 0) {
      pos += snprintf(resp + pos, sizeof(resp) - pos,
                      "{\"siid\":%d,\"piid\":%d,\"code\":0,\"value\":%s}", siid,
                      piid, val);
    } else {
      pos +=
...
/*---------------------------------------------------------------------------
 ...
 - 会先将时间戳按时区偏移修正后再转换
---------------------------------------------------------------------------*/
static void timestamp_to_datetime(long ts, float tz_hours, char *out,
                                  int out_len) {
  ...
  snprintf(out, out_len, "%04d-%02d-%02d %02d:%02d:%02d", year, month, day, hh,
           mm, ss);
}
...
static void handle_time_sync(const char *buf) {
  ...
  strncpy(sys_param.time.date_time, datetime,
          sizeof(sys_param.time.date_time) - 1);
...
static void on_mqtt_message(const char *topic, const char *payload, int len) {
  ...
  DEBUG_4G_PRINTF(
      "[MQTT] >>> on_mqtt_message: topic=%s, len=%d, payload=%s\r\n", topic,
      len, buf);
...
void ml307r_task(void) {
  ...
  // 阶段 0: 设备注册（首次运行）
  if (!reg_done) {
    DEBUG_4G_PRINTF("[ML307R] >>> Stage 0: Device registration\r\n");
    if (!device_register_load_from_flash()) {
      DEBUG_4G_PRINTF(
          "[ML307R] Device not in flash, requesting registration...\r\n");
      device_register_set_info(PRODUCT_ID, PRODUCT_SECRET, PRODUCT_MODEL,
                               PRODUCT_SN);
      if (device_register_request("") == 0) {
        DEBUG_4G_PRINTF("[ML307R] OK - Device registered successfully\r\n");
        reg_done = 1;
      } else {
        DEBUG_4G_PRINTF(
            "[ML307R] !!! Device registration failed, retry in 10s !!!\r\n");
        delay_ms(10000);
        return;
      }
    } else {
      DEBUG_4G_PRINTF("[ML307R] OK - Device credentials loaded from flash\r\n");
      reg_done = 1;
    }
  }
  ...
  // 阶段 1: ML307R 网络初始化
  if (!init_done) {
    DEBUG_4G_PRINTF("[ML307R] >>> Stage 1: ML307R network init\r\n");
    if (ml307r_init() != 0) {
      DEBUG_4G_PRINTF("[ML307R] !!! ml307r_init failed, retry in 5s !!!\r\n");
      delay_ms(5000);
      return;
    }

    // 阶段 2: MQTT 连接
    DEBUG_4G_PRINTF("[ML307R] >>> Stage 2: MQTT connection\r\n");
    if (ml307r_mqtt_connect() != 0) {
      DEBUG_4G_PRINTF(
          "[ML307R] !!! ml307r_mqtt_connect failed, retry in 3s !!!\r\n");
      delay_ms(3000);
      return;
    }

    // 注册下行消息回调
    DEBUG_4G_PRINTF("[ML307R] Registering MQTT callback...\r\n");
    at_mqtt_register_callback(on_mqtt_message);

    // 上报设备信息
    DEBUG_4G_PRINTF("[ML307R] Publishing device info...\r\n");
    publish_device_info();
    init_done = 1;
    DEBUG_4G_PRINTF("[ML307R] OK - All init stages complete!\r\n");
  }
  ...
  if ((uint32_t)(now - last_publish_ms) >= 300000U || now < last_publish_ms) {
    DEBUG_4G_PRINTF(
        "[ML307R] >>> Periodic CT power publish (5min interval)\r\n");
    publish_ct_power();
    last_publish_ms = now;
  }
  ...
  if ((uint32_t)(now - last_online_ms) >= 30000U || now < last_online_ms) {
    DEBUG_4G_PRINTF(
        "[ML307R] >>> Periodic online status publish (30s interval)\r\n");
